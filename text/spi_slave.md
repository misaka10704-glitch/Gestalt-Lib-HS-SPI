...
├── spi_controller            控制器：引脚同步、CS 帧检测、sample/shift 选通、MISO 驱动
├── spi_shift_register        移位寄存器：RX/TX shift、bit 计数、byte_done
└── spi_system_interface      系统接口：RX valid、TX load / valid 握手
```
...
flowchart LR
    subgraph clk_domain["系统时钟域 clk"]
        PHY["spi_controller\n(同步 SCLK/CS/MOSI)"]
        BYTE["spi_shift_register"]
        IF["spi_system_interface"]
...
- 全部逻辑在 **系统时钟 `clk`** 域运行；`sclk`、`cs_n`、`mosi` 经 `spi_controller` 双 flop 同步后再做边沿检测。
- `sample_stb` / `shift_stb` 为 `clk` 域单周期脉冲，由 `CPOL/CPHA` 与 SCLK 边沿组合产生；`spi_shift_register` 据此移位，与 SPI mode 解耦。
- `miso` 在 `spi_controller` 内组合驱动：`cs_active ? tx_bit : 1'b0`；`tx_load` 后首 bit 立即可见。
...
iverilog -o sim.vvp -g2012 \
  src/spi_controller.v src/spi_shift_register.v src/spi_system_interface.v src/spi_slave_top.v \
  tb/tb_spi_slave.v
vvp sim.vvp
