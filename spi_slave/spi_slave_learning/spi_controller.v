...
(
 ...
 input  wire tx_bit //TX移位寄存器的钱bit
...
外部时钟sclk与FPGA内部系统时钟clk无关，因此
若使用clk去采集一个异步信号时，采样时刻可能正好撞
上这个信号的翻转沿；此时触发器输出会进入 亚稳态
（metastability）：既不是稳定的 0 也不是稳定的 1，
而是在 Vih/Vil 之间悬着一个不确定的电压，并且需
要一段随机时间才会"决议"到 0 或 1。

双触发同步器的原理并非消除亚稳态，而是给亚稳态留出
决议时间，把它逃逸到下游的概率压到可忽略。
